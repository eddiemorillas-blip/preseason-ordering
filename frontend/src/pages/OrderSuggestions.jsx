...
const OrderSuggestions = () => {
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      // Pre-calculate splits for each item across all ships
      // Each item starts its distribution at a different ship to balance totals
      const itemSplits = itemsToAdd.map((item, itemIndex) => {
        const totalQty = parseInt(item.quantity) || 0;
        const splits = Array(numShips).fill(0);

        // Start distribution at a different ship for each item (round-robin offset)
        // This ensures later ships get items too, not just remainders
        const startShip = itemIndex % numShips;

        // Distribute each unit starting from the offset ship
        for (let unit = 0; unit < totalQty; unit++) {
          const shipIndex = (startShip + unit) % numShips;
          splits[shipIndex]++;
        }
